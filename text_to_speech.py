...
# buttons
select = Button(root, text="Select PDF", font="lucida 20 bold", fg="black", bg="yellow", command=select_pdf, width=13,
                height=2)
...
l3 = ["LOW", "MEDIUM", "HIGH"]
for i in range(2):
    radio = Radiobutton(root, relief=SUNKEN, text=l1[i], variable=v1, value=l1[i], command=func1).place(x=600,
                                                                                                        y=200 + 30 * i)
for i in range(3):
    radio = Radiobutton(root, relief=SUNKEN, text=l2[i], variable=v2, value=l2[i], command=func2).place(x=800,
                                                                                                        y=200 + 30 * i)
for i in range(3):
    radio = Radiobutton(root, relief=SUNKEN, text=l3[i], variable=v3, value=l3[i], command=func3).place(x=1000,
                                                                                                        y=200 + 30 * i)
